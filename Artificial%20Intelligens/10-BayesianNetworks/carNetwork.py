...
class Variable(object):
    ...
    def calculate_marginal_probability(self):
        ...
        sumFalse = 0
        sumTrue = 0
        for itemKey, itemValue in self.probability_table.items():  #ItemKey = parent state ItemValue = False true probability
            parentProbablity = 1
            for i in range(len(itemKey)):
                parentProbablity *= self.parents[i].get_marginal_probability(itemKey[i])  #parents[i] = parent in question itemKey[i] parent state in given case
            sumFalse += itemValue[0] * parentProbablity
            sumTrue += itemValue[1] * parentProbablity

        self.marginal_probabilities = (sumFalse, sumTrue)
...
class BayesianNetwork(object):
    ...
    def get_joint_probability(self, values: dict):
        ...
        for evidence in values:
            variable = self.get_variable(evidence)
            parentValue = []
            for parent in variable.parents:
                parentValue.append(values[parent.name])
            if len(parentValue) != 0:
                intermediaryProbability = variable.get_probability(values[evidence], tuple(parentValue))
            else:
                intermediaryProbability = variable.get_probability(values[evidence], ())
            P *= intermediaryProbability
        return P
...
def car():
    ...
    damagedTire = Variable('Damaged Tire', ('false', 'true'), dt)
    ElectronicsMalfunction = Variable('Electronics Malfunction', ('false', 'true'), em)
    FuelTankLeaking = Variable('Fuel Tank Leaking', ('false', 'true'), ftl)

    Vibrations = Variable('Vibrations', ('false', 'true'), v, [damagedTire])
    SlowMaxSpeed = Variable('Slow Max Speed', ('false', 'true'), sms, [damagedTire, ElectronicsMalfunction])
    HighConsumption = Variable('High Consumption', ('false', 'true'), hc,
                               [damagedTire, FuelTankLeaking, ElectronicsMalfunction])

    variables = [damagedTire, ElectronicsMalfunction, FuelTankLeaking, Vibrations, SlowMaxSpeed, HighConsumption]
    ...
    print_conditional_probability(network, {'High Consumption': 'true'}, {'Fuel Tank Leaking': 'true'})
    print_conditional_probability(network, {'Fuel Tank Leaking': 'true'}, {'High Consumption': 'true'})
    print('')
